...
/* ─────────────────────────────────────────
   Main Detail Page
───────────────────────────────────────── */
const MuzaffarpurServiceDetail = () => {
  const { id } = useParams();
  ...
  if (!service) {
    return (
      ...
      <EnhancedSEO
        title={`${service?.name || 'Service'} in Muzaffarpur | BookEase Premium`}
        description={`Book ${service?.name || 'premium escort services'} in Muzaffarpur. Verified profile, professional service, 24/7 available. Safe and discreet booking.`}
        ...
        breadcrumbs={[
          ...
          { name: 'Muzaffarpur', url: `https://www.escortmumbaii.in/${window.location.pathname.split('/').slice(0, -2).join('/')}` },
    ...
    <div className="min-h-screen bg-neutral-50 pb-24">
      ...
      <div className="container mx-auto px-4 pt-4">
        <nav className="text-sm text-neutral-500 mb-4 flex items-center gap-1 flex-wrap">
          ...
          <span>›</span>
          <Link to="/bihar/muzaffarpur" className="hover:text-pink-600 transition-colors">Muzaffarpur</Link>
          <span>›</span>
      ...
      <div className="container mx-auto px-4 pb-8">
          ...
          <div className="md:flex">
            ...
            <div className="md:w-1/2 p-6 md:p-8 flex flex-col gap-4">
              ...
              <h1 className="text-2xl md:text-3xl font-bold text-neutral-900 leading-tight">
                {service.name}
                <span className="text-pink-600"> in Muzaffarpur</span>
              </h1>
        ...
        {relatedServices.length > 0 && (
          <section className="mt-10">
            <h2 className="text-2xl font-bold text-neutral-900 mb-5">More Services in Muzaffarpur</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {relatedServices.map((s) => (
                <Link
                  key={s.id}
                  to={`/bihar/muzaffarpur/service/${s.id}`}
...
export default MuzaffarpurServiceDetail;
