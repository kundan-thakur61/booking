...
const GayaServiceDetail = () => {
  ...
  if (!service) {
    return (
      ...
      <EnhancedSEO
        title={`${service?.name || 'Service'} in Gaya | BookEase Premium`}
        description={`Book ${service?.name || 'premium escort services'} in Gaya. Verified profile, professional service, 24/7 available. Safe and discreet booking.`}
        ...
        breadcrumbs={[
          ...
          { name: 'Gaya', url: `https://www.escortmumbaii.in/${window.location.pathname.split('/').slice(0, -2).join('/')}` },
...
export default GayaServiceDetail;
