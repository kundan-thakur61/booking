...
const LowerParelServiceDetail = () => {
    ...
    return (
      <div className="min-h-screen bg-neutral-50">
        ...
        <div className="container mx-auto px-4 py-16 text-center">
          ...
          <Link to="/mumbai/lower-parel" className="bg-pink-600 text-white px-8 py-3 rounded-full font-semibold hover:bg-pink-700 transition-colors">
            Back to Lower Parel
          </Link>
        </div>
      </div>
  ...
  const handleBookNow = () => {
    ...
    navigate('/mumbai/lower-parel/booking');
  };
  ...
  const serviceFaqSchema = {
    ...
    "mainEntity": [
      {
        "@type": "Question",
        "name": `Is ${service.name} available in Lower Parel?`,
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `Yes, ${service.name} is available in Lower Parel, Mumbai. Our verified companions offer this service 24/7 with complete privacy and discretion.`
        }
      },
      {
        "@type": "Question",
        "name": `How do I book ${service.name} in Lower Parel?`,
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `To book ${service.name} in Lower Parel, browse the profile, then contact us via phone or WhatsApp. Our team will help arrange a safe, discreet booking.`
        }
      },
      {
        "@type": "Question",
        "name": "Are profiles verified for Lower Parel?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Yes, all profiles serving Lower Parel are thoroughly verified with ID proof and authentic photos for your safety."
        }
      }
    ]
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      <SEO
        title={`${service.name} in Lower Parel — Verified | BookEase`}
        description={`✓ ${service.name} in Lower Parel, Mumbai ✓ Verified profiles ✓ 24/7 available ✓ Safe & discreet. Book now on BookEase. 18+ only.`}
        canonical={`https://www.escortmumbaii.in/mumbai/lower-parel/service/${id}`}
        currentPath={`/mumbai/lower-parel/service/${id}`}
        entityType="service"
      ...
      <div className="container mx-auto px-4 pt-4">
        <nav className="text-sm text-neutral-500 mb-4">
          ...
          <span className="mx-2">›</span>
          <Link to="/mumbai/lower-parel" className="hover:underline">Lower Parel</Link>
          <span className="mx-2">›</span>
            ...
            <div className="md:w-1/2 flex flex-col">
              ...
              <div className="relative group">
                <LazyImage
                  ...
                  alt={`${service.name} in Lower Parel, Mumbai - Photo ${selectedImg + 1}`}
              ...
              {allImages.length > 1 && (
                <div className="flex gap-2 p-3 overflow-x-auto bg-neutral-100">
                  {allImages.map((img, idx) => (
                    ...
                    >
                      <LazyImage src={img} alt={`${service.name} in Lower Parel - Gallery photo ${idx + 1}`} className="w-full h-full object-cover" />
                    </button>
                  ))}
                </div>
              )}
              ...
              <div>
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex items-center gap-3 text-sm text-neutral-700">
                    ...
                    100% Verified Lower Parel Profiles
                  </div>
                  <div className="flex items-center gap-3 text-sm text-neutral-700">
                    <svg className="w-5 h-5 text-green-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
      ...
      {relatedServices.length > 0 && (
        <div className="container mx-auto px-4 pb-12">
          <h2 className="text-2xl font-bold text-neutral-900 mb-6">
            More Services in Lower Parel
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {relatedServices.map((s) => (
              <Link
                key={s.id}
                to={`/mumbai/lower-parel/service/${s.id}`}
...
export default LowerParelServiceDetail;
