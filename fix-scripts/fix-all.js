...
function getLocals(citySlug) {
  return cityLocals[citySlug] || `${citySlug.replace(/-/g,' ')},city centre,main market`;
}
...
───────────────────────────────────────────────────────────── */
function buildCityPageJsx(stateDir, cityFile, stateLabel, cityLabel, stateSlug, citySlug) {
...
───────────────────────────────────────────────────────────── */
function buildServiceDetailJsx(stateDir, cityFile, stateLabel, cityLabel, stateSlug, citySlug) {
  const componentName = `${cityFile}ServiceDetail`;
  const dataImport = `../../data/${stateDir}/${cityFile}services`;
  const cityPath = `/${stateSlug}/${citySlug}`;
            ...
            <div className="md:w-1/2 p-6 md:p-8 flex flex-col gap-4">
              ...
              {service.aboutme && (
                <div className="bg-pink-50 rounded-xl p-4 border border-pink-100">
                  ...
                  <p className="text-neutral-700 text-sm leading-relaxed">{service.aboutme}</p>
                </div>
              )}
...
for (const stateDir of stateDirs) {
  ...
  if (!fs.existsSync(detailStateDir)) {
    console.log(`  [SKIP] No serviceCarDetails dir for state: ${stateDir}`);
    skipped++;
    continue;
  }

  // Get all city service .js files in data/<State>/
  const dataFiles = fs.readdirSync(dataStateDir).filter(f => f.endsWith('services.js'));

  for (const dataFile of dataFiles) {
    // cityFile = "ConnaughtPlace" from "ConnaughtPlaceservices.js"
    const cityFile  = dataFile.replace('services.js', '');
    const cityLabel = dirToLabel(cityFile);
    const citySlug  = dirToSlug(cityFile);

    // ── FIX Issue 5: Rewrite data file with correct city data ──
    ...
    // ── FIX Issues 2 & 3: Rewrite city listing JSX ──
    const cityPagePath = path.join(detailStateDir, `${cityFile}.jsx`);
    if (fs.existsSync(cityPagePath)) {
      const newCityJsx = buildCityPageJsx(stateDir, cityFile, stateLabel, cityLabel, stateSlug, citySlug);
      fs.writeFileSync(cityPagePath, newCityJsx, 'utf8');
      console.log(`  [JSX]  Fixed: serviceCarDetails/${stateDir}/${cityFile}.jsx`);
      jsxFixed++;
    }

    // ── FIX Issues 2 & 3: Rewrite service detail JSX ──
    const detailPagePath = path.join(detailStateDir, `${cityFile}servicesDetails.jsx`);
    if (fs.existsSync(detailPagePath)) {
      const newDetailJsx = buildServiceDetailJsx(stateDir, cityFile, stateLabel, cityLabel, stateSlug, citySlug);
      fs.writeFileSync(detailPagePath, newDetailJsx, 'utf8');
      console.log(`  [JSX]  Fixed: serviceCarDetails/${stateDir}/${cityFile}servicesDetails.jsx`);
      jsxFixed++;
    }
  }
}
...
console.log('\n════════════════════════════════════');
console.log(`✅ Done!`);
console.log(`   Data files fixed : ${dataFixed}`);
